    ...
    match Mode:
        ...
        case InstructionMode.Immediate:
            ...
            if NumArgs > 0:
                ...
                if RValue.is_err: return Err(RValue.Err())
                else: Value = RValue.unwrap()
            
            if NumArgs > 1:
                RReg = ParseArgument(LineVector[2], ParseMode.Register)
                if RReg.is_err: return Err(RValue.Err())
...
def VerifyOpcode(Instruction: int, ExpectedOpcode: str) -> Result[bool, str]:
    ...
    if ROpcode.is_err: return Err(ROpcode.unwrap_err())
    OpcodeTuple = ROpcode.unwrap()
    ActualOpcodeID = OpcodeTuple[0]
    ExpectedOpcodeID = OPCODES[ExpectedOpcode][0]
    if not ExpectedOpcodeID == ActualOpcodeID:
        return Err("Expected opcode '{}' does not match actual opcode '{}' from instruction {}".format(ExpectedOpcodeID, ActualOpcodeID, Instruction))
    return Ok(True)
...
def ADD(CPU: CPU, Instruction: int) -> Result[CPU, str]:
    ...
    if not Extraction[0] == InstructionMode.Register:
    ...
    # Get each register
    RegA = CPU.RegisterFile.Registers[REGISTERIDS[int(Extraction[1][1])]]
    RegB = CPU.RegisterFile.Registers[REGISTERIDS[int(Extraction[1][2])]]
    RegC = CPU.RegisterFile.Registers[REGISTERIDS[int(Extraction[1][3])]]
    
    # Perform the operation
    Res = RegA + RegB
    
    # Save the result to the source register
    CPU.RegisterFile.SetReg(RegC, Res)
...
def ADDI(CPU: CPU, Instruction: int) -> Result[CPU, str]:
    ...
    print("Performed a '{}' instruction!".format(InstructionMnemonic))
...
def LDI(CPU: CPU, Instruction: int) -> Result[CPU, str]:
    ...
    if not Mode == InstructionMode.Immediate: return Err("Expected nemonic '{}' to be Immediate!".format(InstructionMnemonic))
    
    RExtraction = Extract(Instruction)
    if RExtraction.is_err: return Err(RExtraction.unwrap_err())
    Extraction = RExtraction.unwrap()
    
    if not Extraction[0] == InstructionMode.Immediate:
    ...
    # Save the result to the source register
    CPU.RegisterFile.SetReg(RegA, Value)
