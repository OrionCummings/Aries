...
void test_single_expr_assignment_u8(void) {
    ...
    Expression* e1 = expr_parse(scratchpad, sl1);
    Expression* e2 = expr_parse(scratchpad, sl2);
    Expression* e3 = expr_parse(scratchpad, sl3);

    if (!e1) { TEST_FAIL_MESSAGE("failed to allocate expr 1"); }
    if (!e2) { TEST_FAIL_MESSAGE("failed to allocate expr 2"); }
    if (!e3) { TEST_FAIL_MESSAGE("failed to allocate expr 3"); }

    // TODO/NOTE: This doesn't check the other fields of Expression. Maybe that's a problem?
    if (e1->int_value != 1) { TEST_FAIL_MESSAGE("bad value in expr 1"); }
    if (e2->int_value != 12) { TEST_FAIL_MESSAGE("bad value in expr 2"); }
    if (e3->int_value != 123) { TEST_FAIL_MESSAGE("bad value in expr 3"); }
