#include "test_bint_sub.h"

void run_bint_sub_tests(void) {
    RUN_TEST(test_bint_sub);
    RUN_TEST(test_bint_sub_increment_no_overflow);
    RUN_TEST(test_bint_sub_decrement_no_underflow);
    RUN_TEST(test_bint_sub_increment_overflow);
    RUN_TEST(test_bint_sub_decrement_underflow);
    RUN_TEST(test_bint_sub_overflow_first_byte);
    RUN_TEST(test_bint_sub_overflow_mid_byte);
    RUN_TEST(test_bint_sub_overflow_last_byte);
    RUN_TEST(test_bint_sub_underflow_first_byte);
    RUN_TEST(test_bint_sub_underflow_mid_byte);
    RUN_TEST(test_bint_sub_underflow_last_byte);
}

void test_bint_sub(void) {
    TEST_ASSERT_MESSAGE(false, "NOT IMPLEMENTED");
}

void test_bint_sub_increment_no_overflow(void) {
    TEST_ASSERT_MESSAGE(false, "NOT IMPLEMENTED");
}

void test_bint_sub_decrement_no_underflow(void) {
    TEST_ASSERT_MESSAGE(false, "NOT IMPLEMENTED");
}

void test_bint_sub_increment_overflow(void) {
    TEST_ASSERT_MESSAGE(false, "NOT IMPLEMENTED");
}

void test_bint_sub_decrement_underflow(void) {
    TEST_ASSERT_MESSAGE(false, "NOT IMPLEMENTED");
}

void test_bint_sub_overflow_first_byte(void) {
    TEST_ASSERT_MESSAGE(false, "NOT IMPLEMENTED");
}

void test_bint_sub_overflow_mid_byte(void) {
    TEST_ASSERT_MESSAGE(false, "NOT IMPLEMENTED");
}

void test_bint_sub_overflow_last_byte(void) {
    TEST_ASSERT_MESSAGE(false, "NOT IMPLEMENTED");
}

void test_bint_sub_underflow_first_byte(void) {
    TEST_ASSERT_MESSAGE(false, "NOT IMPLEMENTED");
}

void test_bint_sub_underflow_mid_byte(void) {
    TEST_ASSERT_MESSAGE(false, "NOT IMPLEMENTED");
}

void test_bint_sub_underflow_last_byte(void) {
    TEST_ASSERT_MESSAGE(false, "NOT IMPLEMENTED");
}