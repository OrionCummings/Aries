#include "test_bint_mul.h"

void run_bint_mul_tests(void) {
    RUN_TEST(test_bint_mul);
    RUN_TEST(test_bint_mul_increment_no_overflow);
    RUN_TEST(test_bint_mul_decrement_no_underflow);
    RUN_TEST(test_bint_mul_increment_overflow);
    RUN_TEST(test_bint_mul_decrement_underflow);
    RUN_TEST(test_bint_mul_overflow_first_byte);
    RUN_TEST(test_bint_mul_overflow_mid_byte);
    RUN_TEST(test_bint_mul_overflow_last_byte);
    RUN_TEST(test_bint_mul_underflow_first_byte);
    RUN_TEST(test_bint_mul_underflow_mid_byte);
    RUN_TEST(test_bint_mul_underflow_last_byte);
}

void test_bint_mul(void) {
    TEST_ASSERT_MESSAGE(false, "NOT IMPLEMENTED");
}

void test_bint_mul_increment_no_overflow(void) {
    TEST_ASSERT_MESSAGE(false, "NOT IMPLEMENTED");
}

void test_bint_mul_decrement_no_underflow(void) {
    TEST_ASSERT_MESSAGE(false, "NOT IMPLEMENTED");
}

void test_bint_mul_increment_overflow(void) {
    TEST_ASSERT_MESSAGE(false, "NOT IMPLEMENTED");
}

void test_bint_mul_decrement_underflow(void) {
    TEST_ASSERT_MESSAGE(false, "NOT IMPLEMENTED");
}

void test_bint_mul_overflow_first_byte(void) {
    TEST_ASSERT_MESSAGE(false, "NOT IMPLEMENTED");
}

void test_bint_mul_overflow_mid_byte(void) {
    TEST_ASSERT_MESSAGE(false, "NOT IMPLEMENTED");
}

void test_bint_mul_overflow_last_byte(void) {
    TEST_ASSERT_MESSAGE(false, "NOT IMPLEMENTED");
}

void test_bint_mul_underflow_first_byte(void) {
    TEST_ASSERT_MESSAGE(false, "NOT IMPLEMENTED");
}

void test_bint_mul_underflow_mid_byte(void) {
    TEST_ASSERT_MESSAGE(false, "NOT IMPLEMENTED");
}

void test_bint_mul_underflow_last_byte(void) {
    TEST_ASSERT_MESSAGE(false, "NOT IMPLEMENTED");
}